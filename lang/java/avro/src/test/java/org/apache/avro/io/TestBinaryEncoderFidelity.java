...
public class TestBinaryEncoderFidelity {
	...
	public static void generateData(Encoder e, boolean useReadOnlyByteBuffer)
			throws IOException {
		// generate a bunch of data that should test the bounds of a
		// BinaryEncoder
		...
		if (e instanceof BinaryEncoder) {
			...
			System.out.println(
					e.getClass().getSimpleName() + " buffered: " + count);
		}
		e.flush();
	}
	...
	public static void generateLegacyData() throws IOException {
		...
		Encoder e = new MockLegacyBinaryEncoder(baos).instance;
		// Call the real methods defined in super class
		Mockito.doCallRealMethod().when(e)
				.writeBytes(Mockito.any(byte[].class));
		Mockito.doCallRealMethod().when(e)
				.writeFixed(Mockito.any(byte[].class));
		Mockito.doCallRealMethod().when(e)
				.writeString(Mockito.any(StringBuilder.class));

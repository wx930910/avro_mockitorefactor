...
public class TestGenericDatumWriter {
	...
	public void testArrayConcurrentModification() throws Exception {
		...
		ByteArrayOutputStream bao = new ByteArrayOutputStream();
		final GenericDatumWriter<GenericArray<Integer>> w = new GenericDatumWriter<>(
				s);

		CountDownLatch sizeWrittenSignal = new CountDownLatch(1);
		CountDownLatch eltAddedSignal = new CountDownLatch(1);

		final TestEncoder e = new TestEncoder(
		...
		try {
			...
			assertTrue(
					ex.getCause() instanceof ConcurrentModificationException);
		}
	}
	...
	public void testMapConcurrentModification() throws Exception {
		...
		ByteArrayOutputStream bao = new ByteArrayOutputStream();
		final GenericDatumWriter<Map<String, Integer>> w = new GenericDatumWriter<>(
				s);
	...
	@Test
	public void writeFieldWithDefaultWithExplicitNullDefaultInSchema()
			throws Exception {
	...
	@Test
	public void writeFieldWithDefaultWithoutExplicitNullDefaultInSchema()
			throws Exception {
	...
	private void writeObject(Schema schema, GenericRecord datum)
			throws Exception {

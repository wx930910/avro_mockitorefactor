...
public class AvroJob {
	...
	/**
	 * The configuration key for a job's output compression codec. This takes
	 * one of the strings registered in
	 * {@link org.apache.avro.file.CodecFactory}
	 */
	...
	/**
	 * Configure a job's output schema. Unless this is a map-only job, this must
	 * be a {@link Pair} schema.
	 */
	...
	public static void setOutputMeta(JobConf job, String key, byte[] value) {
		try {
			job.set(BINARY_PREFIX + key, URLEncoder.encode(new String(value, StandardCharsets.ISO_8859_1), StandardCharsets.ISO_8859_1.name()));
	...
	private static void configureAvroInput(JobConf job) {
		if (job.get("mapred.input.format.class") == null) job.setInputFormat(AvroInputFormat.class);

		if (job.getMapperClass() == IdentityMapper.class) job.setMapperClass(HadoopMapper.class);

		configureAvroShuffle(job);
	}

	private static void configureAvroOutput(JobConf job) {
		if (job.get("mapred.output.format.class") == null) job.setOutputFormat(AvroOutputFormat.class);

		if (job.getReducerClass() == IdentityReducer.class) job.setReducerClass(HadoopReducer.class);

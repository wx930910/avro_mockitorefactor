...
public class TestWordCount {
	...
	public static class MockMapImpl {
		public AvroMapper<Utf8, Pair<Utf8, Long>> MockedMapImpl;

		public MockMapImpl() {
			this.MockedMapImpl = EasyMock.partialMockBuilder(AvroMapper.class)
					.addMockedMethod("map", Object.class, AvroCollector.class, Reporter.class).createMock();
			mockMap();
			EasyMock.replay(this.MockedMapImpl);
		}

		private void mockMap() {
			try {
				this.MockedMapImpl.map(EasyMock.anyObject(Utf8.class), EasyMock.<AvroCollector<Pair<Utf8, Long>>> anyObject(),
						EasyMock.anyObject(Reporter.class));
				EasyMock.expectLastCall().andAnswer(() -> {
					Utf8 text = EasyMock.getCurrentArgument(0);
					AvroCollector<Pair<Utf8, Long>> collector = EasyMock.getCurrentArgument(1);
					StringTokenizer tokens = new StringTokenizer(text.toString());
					while (tokens.hasMoreTokens()) {
						Pair<Utf8, Long> p = new Pair<>(new Utf8(tokens.nextToken()), 1L);
						collector.collect(p);
	...
	public static class MockReduceImpl {
		AvroReducer<Utf8, Long, Pair<Utf8, Long>> MockedReduceImpl;

		public MockReduceImpl() {
			this.MockedReduceImpl = EasyMock.partialMockBuilder(AvroReducer.class)
					.addMockedMethod("reduce", Object.class, Iterable.class, AvroCollector.class, Reporter.class).createMock();
			mockReduce();
			EasyMock.replay(this.MockedReduceImpl);
		}

		private void mockReduce() {
			try {
				this.MockedReduceImpl.reduce(EasyMock.anyObject(Utf8.class), EasyMock.anyObject(Iterable.class),
						EasyMock.anyObject(AvroCollector.class), EasyMock.anyObject(Reporter.class));
				EasyMock.expectLastCall().andAnswer(() -> {
					Utf8 word = EasyMock.getCurrentArgument(0);
					Iterable<Long> counts = EasyMock.getCurrentArgument(1);
					AvroCollector<Pair<Utf8, Long>> collector = EasyMock.getCurrentArgument(2);
					long sum = 0;
	...
	public void testJob(String pathOut) throws Exception {
		...
		// new MockMapImpl().MockedMapImpl.getClass()
		AvroJob.setMapperClass(job, new MockMapImpl().MockedMapImpl.getClass());
